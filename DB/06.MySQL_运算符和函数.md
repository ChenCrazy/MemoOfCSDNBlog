...
---------|----------|---------
 ...
 [NOT] LIKE | 模式匹配 | `SELECT 'MySQL' LIKE 'M%'`此处%代表任意零个或多个字符，返回1ture/0false,这种操作当遇到匹配%字面量时可以处理为`SELECT * FROM test WHERE first_name LIKE '%1%%' ESCAPE '1'`使用数字1来告知DBS1后的字符不再是字符的解析匹配而是标准符号,,类似的还有_代表任意一个字符
...
名称 | 描述 | 命令示例
---------|----------|---------
 [NOT] BETWEEN...AND | [不]在闭合区间范围内 | `SELECT 45 [NOT] BETWEEN 22 AND 453;`ture返回1，false返回0
 [NOT] IN() | [不]在列出值范围内 | `SELECT 12 [NOT] IN(45,52,12,23);`
 IS [NOT] NULL  | 是否[不为]空 | `SELECT * FROM test WHERE first_name IS [NOT] NULL;`
## 4.MySQL 日期时间函数(常用) 
